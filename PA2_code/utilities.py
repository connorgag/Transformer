...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        actual_tokens = ([self.tokenizer.decode([i]) for i in padded_sentence])
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            tick_values = range(len(actual_tokens))
            plt.xticks(ticks=tick_values, labels=actual_tokens, rotation='vertical')            
            plt.yticks(ticks=tick_values, labels=actual_tokens, rotation='horizontal')
